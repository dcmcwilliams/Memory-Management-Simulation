...
function Heap() {
	...
	// Use worst-fit method: from the list of holes, Largest hole is allocated
	this.requestAllocation = function(process) {
		blockWorstfit= this.head;

		// Make sure our initial worst block is valid
		...
		//log("Initial worst block: " + blockWorstfit.size);
		...
		while ((block != null)) {
			...
			if ((block.size >= process.size) && (block.available) && (block.size > blockWorstfit.size )) {
				blockWorstfit= block;
				//log("New worst block: " + blockWorstfit.size);
			};
			block = block.next;
		};
