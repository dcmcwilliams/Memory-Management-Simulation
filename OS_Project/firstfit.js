...
function Heap() {
	...
	// Use first-fit method: from the list of holes, choose the first hole
	this.requestAllocation = function(process) {
		blockFirstfit= this.head;

		// Make sure our initial first block is valid
		...
		//log("Initial first block: " + blockFirstfit.size);
		...
		while (block != null) {
			...
			if ((block.size >= process.size) && (block.available)) {
				blockFirstfit= block;
				//log("New first block: " + blockFirstfit.size);
			};
			block = block.next;
		};
